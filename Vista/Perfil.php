...
$asignaciones=$objsm->ConsultaAsignacionUsuario($id);
}
?>
...
<ul class="nav-links" style="display:<?php echo $visibleEmpleadoDep ?>">
  ...
  <section class="home-section">
    ...
    <div style="margin-left:auto;margin-right:auto;width:60%;text-align:center;display:<?php echo $visibleEmpleadoDep ?>">
    ...
    <table>
      ...
      while ($row = mysqli_fetch_array($asignaciones)){
       ...
       if ($vencimiento=='OK'){
        ...
        echo "<tr style='background-color:".$color."'><td style='display:none'>".$row["idempleadoasignacion"]."</td><td>".$row["mensajeasignacion"]."</td><td>".$row["fechalimiteasignacion"]."</td><td> <a onclick='FinalizarAsignacion(". $row["idempleadoasignacion"].")'><i class='bx bxs-check-square bx-sm' ></i></a></td></tr>";
       }
       ?>
...
$("#contenidoGrafico").highcharts(config);
    ...
    function FinalizarAsignacion(idasignacion){
      $.ajax({
                  url: 'PerfilMetodo.php?tipo=finalizarasignacion', 
                  type: 'POST',
                  data:{id:idasignacion}, // solo le mandamos el id, porque no hay formulario
                  success:function(response)
                  {
                      mensaje("La asignación ha sido Finalizada.","success");
                      setTimeout(function(){window.location.href = "Perfil.php";}, 3000);//Redirecciona luego de 3 Segundos para poder visualizar el mensaje
